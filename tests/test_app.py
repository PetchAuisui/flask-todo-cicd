...
class TestTodoAPI:
    ...
    def test_get_all_todos_ordered(self, client, app):
        ...
        with app.app_context():
            t1 = Todo(title='Todo 1', created_at=datetime.utcnow() - timedelta(seconds=2))
            t2 = Todo(title='Todo 2', created_at=datetime.utcnow() - timedelta(seconds=1))
    ...
    def test_get_todos_database_error(self, mock_todo, client):
        ...
        mock_todo.query.order_by.return_value.all.side_effect = SQLAlchemyError('DB Error')
        res = client.get('/api/todos')
        assert res.status_code == 500
        data = res.get_json()
        assert data['success'] is False
...
class TestIntegration:
    ...
    def test_complete_todo_lifecycle(self, client):
        ...
        # Update
        update_res = client.put(f'/api/todos/{tid}', json={
